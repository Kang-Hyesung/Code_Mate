...
<div class="row mainArea">
	...
	<div class="col-12" style="background-color: #f1f3f5; height: 30px; display: flex;">
		...
		<div class="searchbar">"${searchcon } 검색 결과"</div>
	</div>
		  	...
		  	<div class="row searchFormArea align-self-center">
				...
				<div class="col-8">
				    ...
				    <form class="d-flex" role="search" action="search.action" method="get">
				      <input class="form-control me-2 searchTextForm" type="search" placeholder="검색어를 입력하세요." aria-label="Search" value="${searchcon }">
		...
		<div class="row main-text">
			<div class="col-9 sermenu">
				<ul class="col-12 ulul" style="">
					<li class="liview" id="list1">전체(${allcount })</li>
					<li class="lili" id="list2">프로젝트(${pjcount })</li>
					<li class="lili" id="list3">Q&A(${qnacount })</li>
				</ul>
				<div style="display: flex; padding:15px;">
					<div class="col-9">
						...
						<a href="" class="aa">
						 ...
						 -->
						<div id="pjlist">
						<c:forEach var="pjlist" items="${pjlist }" begin="0" end="4" step="1" varStatus="status">
						...
						<a href="" class="aa">
							<div class="col-11" style="border-bottom: 1px solid #f1f3f5;">
								...
								<div class="col-10" style="display: flex; flex-wrap: wrap;">
									<span class="mojip">${pjlist.status }</span><span>｜</span>
									<p class="mojipTitle">${pjlist.title }</p>
									<!-- <span class="new">new</span> -->
								</div>
								<div>
									<p class="mojipContent">${pjlist.content }</p>
								</div>
								<div style="margin-top: 30px;">
									<ul style="display: flex;">
									
									<c:forEach var="tag" items="${tag }">
										<c:if test="${pjlist.ap_code == tag.ap_code }">
								...
								<div>
								<p class="good" style="margin-top: 30px;">마감일 : ${pjlist.sdate }</p>
								<p class="good">작성자😊 : ${pjlist.nickname }</p>
								</div>
								<div style="display: flex; justify-content: space-between;">
									<div style="display: flex;">
									 
									<c:forEach var="pjcomment" items="${pjcomment }">
									<c:if test="${pjlist.ap_code == pjcomment.ap_code }">
										<p class="good">댓글💬 : ${pjcomment.count }</p>
									</c:if>
									</c:forEach>
									
									<p class="comment">조회수👁️ : ${pjlist.views }</p>
									</div>
									<p class="comment">작성일 : ${pjlist.kdate }</p>
								</div>
							</div>
						</a>
						</c:forEach>
						...
						<div id="qnalist">
						<c:forEach var="qnalist" items="${qnalist }" begin="0" end="4" step="1" varStatus="status">
						<c:if test="${status.count==1 }">
							...
							<div style="display: flex;">
								<div class="col-12" style="border-bottom: 1px solid #f1f3f5; display: flex; height: 100%;">
									...
									<div class="col-9 spans">
										...
										<div class="good">작성자😊 : ${qnalist.nickname }</div>
										<div style="display: flex; justify-content: space-between;">
											<div style="display: flex;">
											
											<c:forEach var="qnacomment" items="${qnacomment }">
											<c:if test="${qnalist.tq_code == qnacomment.cq_code }">
												<p class="good">댓글💬 : ${qnacomment.count }</p>
											</c:if>
											</c:forEach>
											
											<p class="comment">조회수👁️ : ${qnalist.views }</p>
											</div>
											<p class="comment">작성일 : ${qnalist.kdate }</p>
										</div>
									</div>
								</div>
							</div>
...
그리고 마지막 부분의 도어락소리가 들릴때 제가 꿈속에서 자의로 생각을 해서(자각몽) 꿈이 깨어난거인가요? 그거에 대한것도 답변 남겨주시면 감사하겠습니다
					...
					<div class="col-3" style="border-left: 2px solid #f1f3f5;">
						...
						<div class="rank" style="border-bottom: 1px solid #f1f3f5;">
						<a href="" class="a1">
							<div style="display: flex;">
								<div><span class="ranktop">${status.count}</span></div>
								<div class="imgbox">
									<img class="rankimg" src="img/poby.png" alt="html" />
								</div>
								<div>
									<span class="ranktop">${hotpj.nickname }</span>
								</div>
							</div>					
